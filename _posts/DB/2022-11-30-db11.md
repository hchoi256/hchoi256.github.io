...
- **No index, unsorted** $$\rightarrow$$ must scan whole table
...
2-1. if duplicates in merging $$\rightarrow$$ ignore it.
...
4. hashing (if partition is still too big $$\rightarrow$$ go to 2 (recursive))
...
- For each tuple, discard unwanted fields, apply hash function $$h1$$ to choose one of $$B-1$$ output buffers.
- Result = $$B-1$$ partitions (of tuples with no unwanted fields)
    - 2 tuples from different partitions guaranteed to be distinct.

#### Duplicate elimination phase**
- For each partition, read it and build an in-memory hash table, using hash fn $$h2\ (<>\ h1)$$ on all fields, while discarding duplicates. 
...
- For each S-partition:
    ...
    - scan correlation with R-partition
    - add tuples to table while discarding duplicates.

> 기말출제!! ★★★★★★
>
> Sort-based intersection to union $$\rightarrow$$ hash-based intersection to union
