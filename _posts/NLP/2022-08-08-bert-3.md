...
categories: NLP
tag: [NLP, ConvS2S, Self-Attention]
toc: true
toc_sticky: true
toc_label: "쭌스log"
...
- *Residual Connection*: 입력 정보가 가중치 합(weight sum) 연산에 적용되고자 route를 만든다. 다음 레이어에 전달될 벡터에 더해져서 최종 출력에 영향을 줄 수 있도록 하기 위함.
...
순환 신경망의 레이어 간 연결선을 제거하여 이전 출력을 기다릴 필요없이, 상기 사진에서는 가령 3개의 단어 토큰 단위로 새로운 단어 벡터를 인풋으로 활용한다.
