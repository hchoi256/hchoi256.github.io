...
3D 좌표를 생성하기 위해서 2D 이미지와 3D Space 사이의 관계를 먼저 이해해야 합니다.

하여 상기 이미지에서 <span style="color:red"> 빨간색 Image Plane 영역</span>의 모든 point들을 3D space로 일대일 대응 projection 시켜서 **Camera Frustum Space**를 생성합니다.

이후, Camera Frustum Space를 Meshgrid로 나누어 각 Point에 대한 3D 좌표를 얻습니다.

<span style="color:yellow"> $$p^{3d}_{i,j}=K^{-1}_i p^m_j$$ </span>

- $$i$$: $$i$$번째 다중 뷰 이미지입니다.

상기 공식을 활용해서 Camera Frustum Space의 3D 좌표를 3D 월드뷰 공간으로 역투영합니다.
