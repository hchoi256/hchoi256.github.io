...
- $x~p_{data}(x)$: 원본 데이터 분포에서 샘플 하나 추출
- $E_{z~p_{z}(z)}[log(1-D(G(z)))]$: noise 분포에서 채취된 각각의 noise를 적용하여 생성한(G) 이미지가 얼마나 진짜같은지(D) 나타내는 척도의 기댓값

궁극적으로, 생성된 이미지가 원본 이미지와 차이가 없어져 D(G(z))은 $1/2$로 수렴하고, 생성 분포(G)는 기존 데이터 분포에 잘 근사할 것이다.
- 여기서, $1/2$이란 0 fake도 아니고 1 real도 아닌 중간지점, 즉 D에 의해** 구별 불가능한** 상태의 G(z)를 의미한다
